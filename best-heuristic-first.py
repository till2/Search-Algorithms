...
class Node:
    def __init__(self, state, parent_node=None, heuristic=None):
        self.state = state
...
def best_heuristic_first(start, goal, rev):
    # SearchGraph = nx.DiGraph()
    start_node = Node(start)
    Frontier = [start_node]
    while(Frontier):
        # print(Frontier[np.argmin([G.nodes[node.state]["heuristic"] for node in Frontier])])
        max_node_index = np.argmin([G.nodes[node.state]["heuristic"] for node in Frontier])
...
if __name__ == '__main__':
    ...
    goal_path = best_heuristic_first(start, goal, rev)
